...
export default function Wizard({API, auth, role, onDone}){
  ...
  async function create(){
    if(!canEdit){ alert('Viewer cannot create presets.'); return }
    setLoading(true)
    const payload = { fiscal_year: Number(year), gdp_growth: Number(gdp), overrides: {avg_ticket: Number(avgTicket), daily_txn: Number(dailyTxn), labor_pct: Number(laborPct)} }
    const r = await fetch(`${API}/preset/qsr`, auth({method:'POST', body: JSON.stringify(payload)}))
    const js = await r.json(); setLoading(false); onDone?.(year)
  }
  return (
    <div>
