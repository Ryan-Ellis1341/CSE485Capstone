"""
FP&A Fullstack Backend (FastAPI) — v2
- Auth & Roles (Bearer token): Admin, Analyst, Viewer
- Budgets, Scenarios, GDP-driven presets
- AI Forecasts (ARIMA/ML), AI variance explanations
- Board pack export (pptx)
- QuickBooks mapper stub (CSV/JSON → standardized accounts)
- PyTest harness included in /tests
...
# In-memory user store: token -> role
# In production use JWTs; here we keep it simple.
TOKENS = {
...
# FastAPI
# -------------------
api = FastAPI(title="FP&A Backend v2")
...
def login(payload: LoginReq):
    # In a real system, validate password. Here, hand out a role-mapped token.
...
def bva_analyze(payload: BvAReq, user=Depends(require_role("Admin","Analyst","Viewer"))):
    ...
    hot = (bva.assign(abs_var=lambda x: x['variance'].abs()).sort_values('abs_var', ascending=False).head(7)
...
# Expect either JSON rows or a CSV upload with columns: Date, Account, Dept, Amount, Currency
...
def qb_import_json(payload: QBImportJSON, user=Depends(require_role("Admin","Analyst"))):
    ...
    # Map QB 'Account' into our standardized chart
    mapping = {
    ...
    # Merge into actuals (append demo)
    global actuals_df
...
def qb_import_csv(file: UploadFile = File(...), user=Depends(require_role("Admin","Analyst"))):
    ...
    rows = list(reader)
    return qb_import_json(QBImportJSON(rows=rows), user)  # reuse logic
