...
export default function ReportDesigner({api, token}){
  ...
  const [blocks, setBlocks] = useState([
    {type:'title', text:'Q2 Performance Highlights', x:1, y:1, w:8, h:1},
    {type:'text', text:'Margins improved in May due to COGS control.', x:1, y:2, w:8, h:2}
  ])
  const [file, setFile] = useState('')

  function updateBlock(i,k,v){
    setBlocks(bs => bs.map((b,ix)=> ix===i ? {...b, [k]: v} : b))
  }

  async function render(){
    const client = makeClient(api, token)
    const { data } = await client.post('/reports/render', { name, blocks })
    setFile(data.file)
  }
        ...
        {blocks.map((b,i)=>(
          <div key={i} className="border rounded p-2">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              <select value={b.type} onChange={e=>updateBlock(i,'type',e.target.value)} className="border rounded px-2 py-1">
