...
export default function PresetWizard({api, token, year, scenario}){
  ...
  async function run(){
    ...
    const { data } = await client.post('/presets/qsr', form)
    setCount(data.rows)
    ...
    <div className="bg-white rounded-2xl shadow p-4 space-y-3">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {Object.keys(form).map(k=> (
          <div key={k} className={k==='apply'?'col-span-2':''}>
            <label className="text-sm">{k}</label>
            {typeof form[k]==='boolean' ? (
              <label className="flex items-center gap-2">
                <input type="checkbox" checked={form[k]} onChange={e=>on(k, e.target.checked)} />
                Apply to scenario
              </label>
            ) : (
              <input className="border rounded px-2 py-1 w-full" value={form[k]} onChange={e=>on(k, e.target.value)} />
            )}
          </div>
        ))}
      </div>
