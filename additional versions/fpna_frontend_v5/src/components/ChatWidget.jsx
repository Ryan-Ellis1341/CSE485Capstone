...
export default function ChatWidget({api, token, userId="student", threadId="global"}){
  ...
  async function loadHistory(){
    try{
      ...
      const { data } = await client.get('/chat/messages', { params: { thread_id: threadId } })
  ...
  async function send(text){
    ...
    try{
      if(wsRef.current && wsRef.current.readyState === WebSocket.OPEN){
        ...
        const client = makeClient(api, token)
        await client.post('/chat/messages', m)
        setMsgs(x => [...x, { ...m, ts: Date.now()/1000 }])
      }
      setInput("")
    }catch(e){ console.error(e) }
  }
  ...
  return (<>
    <button onClick={()=>setOpen(v=>!v)} className="fixed bottom-4 right-4 px-4 py-2 rounded-full shadow bg-slate-900 text-white">
      {open ? 'Close Chat' : 'Chat'}
    </button>
    {open && (
      <div className="fixed bottom-20 right-4 w-96 bg-white border rounded-xl shadow-lg flex flex-col">
        <div className="px-3 py-2 border-b font-medium">Support Chat</div>
        <div id="chat-scroll" className="p-3 space-y-2 overflow-auto" style={{maxHeight: '50vh'}}>
          ...
          {!msgs.length && <div className="text-sm text-slate-500">No messages yet. Say hi!</div>}
        </div>
        <div className="p-2 border-t flex gap-2">
          <input className="border rounded px-2 py-1 flex-1" value={input} onChange={e=>setInput(e.target.value)} placeholder="Type a message..." />
          <button className="px-3 py-1 rounded bg-slate-900 text-white" onClick={()=>send(input)} disabled={!input.trim()}>Send</button>
        </div>
      </div>
    )}
  </>)
