...
export default function Migrate({api, token, year, scenario}){
  ...
  async function upload(){
    ...
    const url = system==='quickbooks'? '/migrate/quickbooks/csv' : '/migrate/netsuite/csv'
    ...
    <div className="bg-white rounded-2xl shadow p-4 space-y-3">
      ...
      {result && (
        <div className="text-sm">
          ...
          <pre className="bg-slate-100 rounded p-2 mt-2 overflow-auto">{JSON.stringify(result.sample, null, 2)}</pre>
        </div>
      )}
    </div>
  )
