...
export default function Login({api,setApi,setToken,setPage}){
  ...
  async function doLogin(){
    try{
      const client = makeClient(testApi)
      const { data } = await client.post('/auth/login', { email, password })
      setApi(testApi); setToken(data.token); setPage('Dashboard')
  ...
  return (
    <div className="max-w-md mx-auto mt-10 bg-white rounded-2xl shadow p-6 space-y-3">
      ...
      <div className="text-xs text-slate-500">Try admin/analyst/viewer users seeded in backend tests.</div>
    </div>
  )
}